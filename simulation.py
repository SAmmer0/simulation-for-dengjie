...
def read_file(file_name):
    ...
    return diff_mgs, sd_msg, price_a, price_h, time_msg
...
if __name__ == '__main__':
    # 从文件中获取数据
    diff_msgs, sd_msgs, price_as, price_hs, time_msgs = read_file('data.xls')
    str_times = trans_time(time_msgs)
    stocks = {'a': price_as, 'h': price_hs}

    # 设置阀值
    constrain = 3.0     # 终止的上下限
    trigger = 0.75      # 触发建仓的阀值
    ...
    def trade_iter(diff_msg, sd_msg, stocks_price, str_time):
        ...
        for trade_date in range(i):
            diff_divide_sd = diff_msg[trade_date] / sd_msg[trade_date]        # 计算价差序列是标准差的多少倍
            list_diff_divide_sd.append(diff_divide_sd)      # 将比率加入列表，供绘图使用
            ...
            if not in_trade:        # 建仓
                ...
                if trade_date == i - 1:     # 如果已经到了模拟的最后一天不管如何都收盘
    ...
    diff_divide_sd_curve = np.array(list_diff_divide_sd)        # 将价差序列除以标准差的结果列表转换为np列表
    diff_curve = np.array(diff_msgs)        # 将价差序列列表转换为np列表
    up_constrain_curve = np.array([constrain] * len(str_times))     # 设置平仓上界列表
    up_trigger_curve = np.array([trigger] * len(str_times))     # 设置建仓上界列表
    zero_curve = np.array([0] * len(str_times))         # 设置横轴列表
    bottom_trigger_curve = np.array([-trigger] * len(str_times))        # 设置建仓下界列表
    bottom_constrain_curve = np.array([-constrain] * len(str_times))        # 设置平仓下界列表
    ...
    pl.ylim(min(diff_divide_sd_curve.min(), diff_curve.min()) * 1.5,        # 设置y轴的画图范围
            max(diff_divide_sd_curve.max(), diff_curve.max()) * 1.2)
    pl.yticks([-constrain, -trigger, 0, trigger, constrain],        # 设置y轴的标记
              [r'$-constrain$', r'$-trigger$', r'$0$', r'$trigger$', r'$constrain$'])

    # 画出价差序列除以标准差的结果曲线
    pl.plot(x_axle, diff_divide_sd_curve, color='blue', linewidth=2.5,
            linestyle='-', label='diff_divide_sd_curve')
    # 画出价差序列曲线
    pl.plot(x_axle, diff_curve, color='yellow', linewidth=2.5,
            linestyle='-', label='diff_curve')
    # 画出平仓上界曲线
    pl.plot(x_axle, up_constrain_curve, color='red', linewidth=1, linestyle='--')
    # 画出开仓上界曲线
    pl.plot(x_axle, up_trigger_curve, color='green', linewidth=1, linestyle='--')
    # 画出横轴
